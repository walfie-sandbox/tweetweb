...
class UsersServiceSpec extends Specification with Mockito {
  "SlickUsersService" >> {
    ...
    "deleteOld" should {
      "delete users older than specified DateTime" in new WithApplication(fakeApp) {
        val dao = new DAO(DB.driver)
        val usersService = new SlickUsersService(DB, dao)

        val user1 = User(id = "1", updatedAt = new DateTime(0))
        val user2 = User(id = "2", updatedAt = new DateTime(500))
        val user3 = User(id = "3", updatedAt = new DateTime(1000))
        usersService.saveAll(List(user1, user2, user3))
        usersService.find(List("1", "2", "3")) must_== List(user1, user2, user3)

        usersService.deleteOld(new DateTime(500))
        usersService.find(List("1", "2", "3")) must_== List(user2, user3)
      }
    }
  }
}
